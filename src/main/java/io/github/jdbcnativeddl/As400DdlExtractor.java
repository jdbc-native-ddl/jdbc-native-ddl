            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    if ("YES".equalsIgnoreCase(isIdentity)) {
                        ...
                        long identityStart = rs.getLong("IDENTITY_START");
                        long identityIncrement = rs.getLong("IDENTITY_INCREMENT");
                        if ("ALWAYS".equalsIgnoreCase(generation)) {
                            ddl.append(" GENERATED ALWAYS AS IDENTITY");
                        } else {
                            ddl.append(" GENERATED BY DEFAULT AS IDENTITY");
                        }
                        ddl.append(" (START WITH ").append(identityStart)
                           .append(", INCREMENT BY ").append(identityIncrement).append(")");
                    } else {
